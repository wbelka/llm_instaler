...
4. Алгоритмы работы
Алгоритм 1: Определение типа модели
Запрос метаданных: Скачиваются только конфигурационные файлы (config.json, model_index.json) и список файлов из репозитория модели на HuggingFace.

Конвейер Детекторов: Запускаются по очереди детекторы:

GGUFDetector: Ищет файлы .gguf.

DiffusionDetector: Проверяет model_index.json.

SentenceTransformerDetector: Анализирует архитектуру в config.json.

TransformerDetector (по умолчанию): Срабатывает, если другие не определили тип.

Результат: Создается "Профиль Модели" — объект с её типом и базовыми требованиями ({type: "Transformer", library: "transformers"}), который используется на всех последующих этапах.
