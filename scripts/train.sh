...
if [ "$HELP" = true ] || [ -z "$DATA_PATH" ]; then
    ...
    echo "                                 Supported file types: .json, .jsonl, .csv, .txt, .parquet, .yaml, .yml, .toml, .xml, .tsv"
    echo ""
    echo "TRAINING MODES:"
    echo "  --mode MODE                    Training mode: quick, balanced, quality, ultra (default: auto-detect)"
    ...
    echo "  --optimizer OPTIMIZER          Optimizer to use (e.g., adamw_torch, adamw_bnb_8bit, dadapt_adam)"
    echo ""
    ...
    echo "  $0 --data data.json --mode ultra           # Maximum quality, slowest"
    echo ""
    ...
    echo ""
    echo "SUPPORTED DATASET FORMATS:"
    echo "  File formats: JSON, JSONL, CSV, TXT, Parquet, YAML, TOML, XML, TSV"
    echo ""
    echo "  Data formats (auto-detected):"
    echo "    - alpaca: instruction/input/output"
    echo "    - sharegpt: conversations with from/value" 
    echo "    - openai: messages with role/content"
    echo "    - completion: prompt/completion"
    echo "    - qa: question/answer (with optional context)"
    echo "    - text: plain text"
    echo "    - chat: user/assistant"
    echo "    - multi_turn: turns"
    echo "    - vision: image/caption"
    echo "    - vision_qa: image/question/answer"
    echo "    - dolly: instruction/context/response"
    echo "    - oasst: message_id/parent_id/text/role"
    echo "    - vicuna: id/conversations"
    echo "    - wizardlm: instruction/output"
    echo "    - orca: system_prompt/question/answer"
    echo "    - guanaco: text (with special formatting)"
    echo "    - lima: conversations (high quality)"
    echo "    - anthropic: human/assistant"
    echo "    - claude: Human/Assistant"
    echo "    - mistral: text (with special tokens)"
    echo "    - chatml: messages (ChatML format)"
    echo "    - llama_chat: system/user/assistant"
    echo "    - function_calling: functions/function_call"
    echo "    - code_alpaca: instruction/input/output/lang"
    echo ""
    echo "NOTES:"
